...
// Mapeamento dos tamanhos de rolos contínuos
const continuousRolls = [
  LabelSize.LabelSizeRollW12,
  LabelSize.LabelSizeRollW29,
  LabelSize.LabelSizeRollW38,
  LabelSize.LabelSizeRollW50,
  LabelSize.LabelSizeRollW54,
  LabelSize.LabelSizeRollW62,
  LabelSize.LabelSizeRollW62RB,
  LabelSize.LabelSizeRollW102,
  LabelSize.LabelSizeRollW103,
  LabelSize.LabelSizeDTRollW90,
  LabelSize.LabelSizeDTRollW102,
  LabelSize.LabelSizeDTRollW102H51,
  LabelSize.LabelSizeDTRollW102H152,
];

const _printImage = ReactNativeBrotherPrinters?.printImage;
 ...
 * @param params.autoCut            Boolean if the printer should auto cut the receipt/label
 * @param params.labelSize          Label size that we are printing with
 * @param params.paperKind          'diecut' or 'continuous'
 * @param params.isHighQuality
 * @param params.isHalftoneErrorDiffusion
 *
 * @return {Promise<*>}
 */
export async function printImage(device, uri, params = {}) {
  if (!params.labelSize && !params.paperKind) {
    return new Error(
      "Label size or paper kind must be given when printing a label"
    );
  }
  ...
  // Verifica se o labelSize passado é um dos rolos contínuos
  if (continuousRolls.includes(params.labelSize)) {
    newParams.paperKind = "continuous";
  } else {
    // Se a propriedade 'labelSize' existir, mas não for um rolo contínuo,
    // consideramos que é uma etiqueta die-cut.
    if (params.labelSize !== undefined) {
      newParams.paperKind = "diecut";
    }
  }

  return _printImage(device, uri, newParams);
}
