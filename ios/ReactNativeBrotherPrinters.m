...
RCT_REMAP_METHOD(printImage, deviceInfo:(NSDictionary *)device printerUri: (NSString *)imageStr printImageOptions:(NSDictionary *)options resolver:(RCTPromiseResolveBlock)resolve rejecter:(RCTPromiseRejectBlock)reject)
{
    ...
    // Cria um objeto de configurações de impressão padrão para o modelo da impressora.
    BRLMPrintSettings *printSettings = [[BRLMPrintSettings alloc] initDefaultPrintSettingsWithPrinterModel:model];

    // Define o tipo de papel como 'rolo contínuo'.
    // Isso evita o erro de incompatibilidade, pois corresponde à mídia "62mmRB".
    if ([options[@"paperKind"] isEqual: @"continuous"]) {
        printSettings.paperKind = BRLMPrintSettingsPaperKindContinuous;
    }
    
    // Aplica as outras configurações de forma genérica.
    if (options[@"autoCut"]) {
        printSettings.autoCut = [options[@"autoCut"] boolValue];
    }
    
    if (options[@"isHighQuality"]) {
        if ([options[@"isHighQuality"] boolValue]) {
            printSettings.printQuality = BRLMPrintSettingsPrintQualityBest;
            NSLog(@"High Quality is enabled");
        } else {
            printSettings.printQuality = BRLMPrintSettingsPrintQualityFast;
            NSLog(@"High Quality is disabled");
        }
    }
    ...
    if (options[@"isHalftoneErrorDiffusion"]) {
        if ([options[@"isHalftoneErrorDiffusion"] boolValue]) {
            printSettings.halftone = BRLMPrintSettingsHalftoneErrorDiffusion;
            NSLog(@"Error Diffusion is enabled");
        } else {
            printSettings.halftone = BRLMPrintSettingsHalftoneThreshold;
            NSLog(@"Error Diffusion is disabled");
        }
    }
    ...
    NSLog(@"Auto Cut: %@, paperKind: %@", options[@"autoCut"], options[@"paperKind"]);
